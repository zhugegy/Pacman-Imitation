...
int CGYAStarNavi::get_path(int nStartCoordX, int nStartCoordY, int nEndCoordX, int nEndCoordY, CGYEasyStack & CoordStack)
{
  ...
  while (true)
  {
    ...
    {
      tmpRecord = find_the_slot_of_a_node(OpenList, constnListSlotCount, TmpOrigin.nCoordX + (TmpOrigin.nCoordY - 1) * m_nMazeWidth);
      if (tmpRecord == -1)
      {
        //加入
      ...
      {
        if (TmpOrigin.nGCost + 10 < OpenList[tmpRecord].nGCost)
        {
          OpenList[tmpRecord].nGCost = TmpOrigin.nGCost + 10;
          OpenList[tmpRecord].nFather = TmpOrigin.nID;
        }
      }
      ...
      {
        nFlagLeftUp = 1;
        tmpRecord = find_the_slot_of_a_node(OpenList, constnListSlotCount, TmpOrigin.nCoordX - 1 + (TmpOrigin.nCoordY - 1) * m_nMazeWidth);
        if (tmpRecord == -1)
        {
          //加入
        ...
        {
          if (TmpOrigin.nGCost + 14 < OpenList[tmpRecord].nGCost)
          {
            OpenList[tmpRecord].nGCost = TmpOrigin.nGCost + 14;
            OpenList[tmpRecord].nFather = TmpOrigin.nID;
          }
        }
      }
    ...
    {
      tmpRecord = find_the_slot_of_a_node(OpenList, constnListSlotCount, TmpOrigin.nCoordX - 1 + TmpOrigin.nCoordY * m_nMazeWidth);
      if (tmpRecord == -1)
      {
        //加入
    ...
    tmpSmallestF = find_the_smallest_f_in_list(OpenList, constnListSlotCount);

    TmpOrigin = OpenList[tmpSmallestF];
